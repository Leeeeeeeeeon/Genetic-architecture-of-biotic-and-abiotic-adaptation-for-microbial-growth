...
init.est<-function(dat){
  ...
  cm <- colMeans(dat$sp.p)
  
  sum.curve<-function(par6){
    y<-com.get_mu1(par6[c(1,2,3)],times,x0=8.517193)-com.get_mu2(par6[c(4,5)],times,x0=0.0001)
    A <- sum((cm-y)^2)
...
curve.mlefunc<-function( par,y1,times)
{
  ...
  mu <- com.get_mu1(curve.par[c(1,2,3)],times,x0=8.517193)-com.get_mu2(curve.par[c(4,5)],times,x0=0.0001)
...
H0.est<-function(dat){
  ...
  mpheno<-colMeans(dat$sp.p)
  par<-c(0.1626455,26.3409465,-1.4247136,-1.3906243,6.4909985)
  ...
  while(loop_k<max_iter && max_err>epsi){
    ...
    mle.covar1 <- function(npar){
      ...
      AA <- curve.mlefunc(nnpar,y=dat$sp.p,times =times)
      AA
    }
  ...
  LL <- curve.mlefunc(parin2,y=dat$sp.p,times =times)
  return(c(LL,parin2))
}
