...
sum.curve<-function(par6){
  y<-com.get_mu2(par6,times,x0=0.01)
  A <- sum((cm-y)^2)
  A
}


init.est<-function(ct6){
  spp<-ct6
  ty <- cbind(spp)
  cm <- Mct6
  
  sum.curve<-function(par6){
    y<-com.get_mu1(par6[c(1,2,3)],times,x0=0.08273505)-com.get_mu2(par6[c(4,5)],times,x0=0.01)
    A <- sum((cm-y)^2)
    A
  }
  ...
  a<-optim(init.par,sum.curve,method = "Nelder-Mead",control = list(maxit=100000))
  return(a)
}
mu.g<-c(0.1715807,1.5325498,0.8016161,-0.9025740,0.5203707)
init.est(ct6)#c(0.39168782,15.12301265,0.04350461)
mu.g<-init.est(ct6)$par

plot(times,Mct6,xlim = c(0,48),ylim = c(-1,2))
par(new=T)
plot(times,com.get_mu1(mu.g[c(1,2,3)],times,x0=0.08273505),xlim = c(0,48),ylim = c(-1,2),type = "l",col = "red")
par(new=T)
plot(times,-com.get_mu2(mu.g[c(4,5)],times,x0=0.01),xlim = c(0,48),ylim = c(-1,2),type = "l",col = "blue")
par(new=T)
plot(times,com.get_mu1(mu.g[c(1,2,3)],times,x0=0.08273505)-com.get_mu2(mu.g[c(4,5)],times,x0=0.01),xlim = c(0,48),ylim = c(-1,2),type = "l",col = "green")
par(new=T)

k <- length(mu.g)
n <- length(times)
y <- Mct6
SSR0<-init.est(ct6)$value
#SSR0<-sum((Mct6-(com.get_mu1(mu.g[c(1,2,3)],times,x0=0.1013871)-com.get_mu2(mu.g[c(4,5,6)],times,x0=0.01)))^2)
...
curve.mlefunc<-function( par,y1,times)
{
  ...
  mu <- com.get_mu1(curve.par[c(1,2,3)],times,x0=0.08273505)-com.get_mu2(curve.par[c(4,5)],times,x0=0.01)
...
H0.est<-function(ct6){
  ...
  par<-c(0.1715807,1.5325498,0.8016161,-0.9025740,0.5203707)
   covar.par<-c(0.8650835,0.2849479)
  ...
  while(loop_k<max_iter && max_err>epsi){
    ...
    mle.covar1 <- function(npar){
      ...
      AA <- curve.mlefunc(nnpar,y=ct6,times =times)
      AA
    }
    ...
    r1 <- optim(c(parin[3:7]),mle.1,method = "Nelder-Mead",control=list(maxit=1000000))    
  ...
  LL <- curve.mlefunc(parin2,y=ct6,times =times)
  return(c(LL,parin2))
}
...
H1.est <- function(y11,SNP1,init.par=par1,times){
  ...
  for(j in 1:length(snp.type)){
    ...
    sum.curve<-function(par6){
      y<-com.get_mu1(par6[c(1,2,3)],times,x0=0.08273505)-
        com.get_mu2(par6[c(4,5)],times,x0=0.01)
      A <- sum((yy-y)^2)
      A
    }
    r1<-optim(par1[3:7],sum.curve,method = "Nelder-Mead",control = list(maxit=32000))
    snp.index[[j]] <- index
...
com.DH.est1 <- function(ct6,interval=c(1,25173)){
