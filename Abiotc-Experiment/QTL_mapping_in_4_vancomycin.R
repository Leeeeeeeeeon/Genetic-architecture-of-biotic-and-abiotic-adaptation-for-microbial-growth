...
init.est<-function(ct4){
  spp<-ct4
  ty <- cbind(spp)
  cm <- Mct4
  
  sum.curve<-function(par6){
    y<-com.get_mu1(par6[c(1,2,3)],times,x0=0.09681171)-com.get_mu2(par6[c(4,5)],times,x0=0.01)
    A <- sum((cm-y)^2)
    A
  }
...
curve.mlefunc<-function( par,y1,times)
{
  ...
  mu <- com.get_mu1(curve.par[c(1,2,3)],times,x0=0.09681171)-com.get_mu2(curve.par[c(4,5)],times,x0=0.01)
...
#mu.g<-c(0.18758031,1.45563913,0.65809620,-0.88960388,0.27791915,0.01518723)#AIC=-111.074
#mu.g<-c(0.17726562,1.61807726,0.84879829,-0.55732815,0.20252599,0.01603748)
...
H0.est<-function(ct4){
  ...
  par<-c(0.1841994,1.5375224,0.9428398,-1.3943998,0.4986890)
  ...
  while(loop_k<max_iter && max_err>epsi){
    ...
    mle.covar1 <- function(npar){
      ...
      AA <- curve.mlefunc(nnpar,y=ct4,times =times)
      AA
    }
  ...
  LL <- curve.mlefunc(parin2,y=ct4,times =times)
  return(c(LL,parin2))
}
...
com.DH.est1 <- function(ct4,interval=c(1,25173)){
