...
init.est<-function(ct2){
  spp<-ct2
  ty <- cbind(spp)
  cm <- Mct2
  
  sum.curve<-function(par6){
    y<-com.get_mu1(par6[c(1,2,3)],times,x0=0.1013871)-com.get_mu2(par6[c(4,5)],times,x0=0.01)
    A <- sum((cm-y)^2)
    A
  }
...
curve.mlefunc<-function( par,y1,times)
{
  ...
  mu <- com.get_mu1(curve.par[c(1,2,3)],times,x0=0.1013871)-com.get_mu2(curve.par[c(4,5)],times,x0=0.01)
...
H0.est<-function(ct2){
  ...
  par<-c(0.1983664,1.6388763,0.7460471,-2.8538987,0.7509279)
  ...
  while(loop_k<max_iter && max_err>epsi){
    ...
    mle.covar1 <- function(npar){
      ...
      AA <- curve.mlefunc(nnpar,y=ct2,times =times)
      AA
    }
  ...
  LL <- curve.mlefunc(parin2,y=ct2,times =times)
  return(c(LL,parin2))
}
...
com.DH.est1 <- function(ct2,interval=c(1,25173)){
