...
sum.curve<-function(par6){
  y<-com.get_mu1(par6[c(1,2,3)],times,x0=0.04714377)
  A <- sum((cm-y)^2)
  A
}
#init.par<-c(0.1812477,1.5563785,-0.1750842)
init.par<-mu.g
...
init.par<-c(-1.2597041112,1.0055661567,-0.0002741445)
...
H0.est<-function(ct6){
  ...
  #par<-c(0.281851949,1.548488657,1.074343413,-0.516609571,0.429089161,0.005834601)
  #par<-c(0.208882453,1.623298158,1.144577359,-0.680698891,0.454663137,0.005187135)
  # par<-r1$par
...
H1.est <- function(y11,SNP1,init.par=par1,times){
  ...
  while(loop_k<max_iter && max_err>epsi){
    ...
    r1.covar <- optim(parin[1:2],mle.covar1,method = "BFGS",control=list(maxit=2000))
    new.covar1 <- r1.covar$par
